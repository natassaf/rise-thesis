...
fn initialize_model() -> Result<()> {
    let model_path =
        "src/optimized_scheduling_preprocessing/memory_prediction/memory_model/memory_model.onnx";
...
pub async fn predict_memory(memory_features: &Vec<f32>) -> f64 {
    ...
    let memory_features_scaler_path: &str =
        "src/optimized_scheduling_preprocessing/memory_prediction/memory_model/scaler_x.json";
    let memory_target_scaler_path: &str =
        "src/optimized_scheduling_preprocessing/memory_prediction/memory_model/scaler_y.json";
    ...
    let prediction_denormalized =
        memory_target_scaler.inverse_transform(&prediction_normalized_vec);
...
pub async fn predict_memory_batch(memory_features_batch: &[Vec<f32>]) -> Vec<f64> {
    ...
    let predictions_denormalized =
        memory_target_scaler.inverse_transform_batch(&predictions_normalized);
...
mod tests {
    ...
    async fn test_predict_memory() {
        let memory_features: Vec<f32> = vec![
            5437617119388410010.0,
            689248.0,
            38.0,
            58.0,
            110.0,
        ...
        assert!(
            (prediction - expected_prediction).abs() < 0.01,
            "Prediction {} does not match expected {} (diff: {})",
            prediction,
            expected_prediction,
            (prediction - expected_prediction).abs()
        );
    }
}
